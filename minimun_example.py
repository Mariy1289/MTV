...
from scenic.projects.mtv.configs.kinetics.k400_mtv_b2_cva import get_config
...
def main():
    ...
    # end
    return MTV(
        view_configs=config.model.view_configs,
        cross_view_fusion=config.model.cross_view_fusion,
        temporal_encoding_config=config.model.temporal_encoding_config,
        global_encoder_config=config.model.global_encoder_config,
        input_token_temporal_dims=get_input_token_temporal_dims(
            config.dataset_configs.num_frames,
            config.model.view_configs),
        # num_classes=dataset_meta_data['num_classes'],
        num_classes = 400,
        classifier=config.model.classifier,
        dropout_rate=config.model.get('dropout_rate', 0.0),
        attention_dropout_rate=config.model.get('attention_dropout_rate',
                                                     0.1),
        stochastic_depth=config.model.get('stochastic_depth', 0.0),
        keep_spatiotemporal_features=config.model.get(

            'keep_spatiotemporal_features', False),
        final_endpoint=config.model.get('final_endpoint', 'logits'),
        dtype=model_dtype)
...
if __name__ == '__main__':
  ...
  model = main()
  variables = model.init(random.PRNGKey(0), jnp.ones((1,5,3,224,224)))
  breakpoint()
  y = model.apply(variables, jnp.ones((1,5,3,224,224)))
